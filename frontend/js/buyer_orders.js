 ...
 * Handles buyer dashboard functionality for tracking purchases
 */
...
async function loadOrders() {
    try {
        ...
        const response = await fetch(`${API_BASE}/api/orders/buyer/my-orders`, {
            headers: {
...
function createOrderCard(order) {
    ...
    if (order.orderStatus === 'paid') {
        statusMessage = '<div class="order-info-row"><span class="order-info-value" style="color: #F59E0B;">⏳ Waiting for seller to process</span></div>';
    } else if (order.orderStatus === 'processing') {
        statusMessage = '<div class="order-info-row"><span class="order-info-value" style="color: #3B82F6;">📦 Seller is preparing your order</span></div>';
    } else if (order.orderStatus === 'delivered') {
        statusMessage = '<div class="order-info-row"><span class="order-info-value" style="color: #10B981;">✓ Order has been delivered</span></div>';
    }
    ...
    if (order.orderStatus === 'paid') {
        actions = `
            ...
            <button class="btn btn-secondary btn-small" onclick="openCancelModal('${order.id}')">Cancel Order</button>
        `;
    } else if (order.orderStatus === 'delivered') {
        actions = `
 ...
 */
window.openConfirmDeliveryModal = function(orderId) {
    currentOrderId = orderId;
    document.getElementById('confirmDeliveryModal').classList.add('show');
};

/**
 * Close confirm delivery modal
 */
...
window.confirmReceipt = async function() {
    try {
        ...
        const token = await getIdToken();
        const response = await fetch(`${API_BASE}/api/orders/${currentOrderId}/buyer/confirm-delivery`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        showToast('Order completed! Thank you for your confirmation.', 'success');
        await loadOrders();
        ...
        document.getElementById('confirmReceiptBtn').textContent = 'Yes, I Received It';
    }
};
...
window.confirmCancel = async function() {
    try {
        ...
        const token = await getIdToken();
        const response = await fetch(`${API_BASE}/api/orders/${currentOrderId}/buyer/cancel`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        <div class="detail-section">
            ...
            <div class="detail-grid">
                <div class="detail-item">
                    ...
                    <span class="detail-value">${order.paymentMethod ? order.paymentMethod.toUpperCase() : 'N/A'}</span>
                </div>
                <div class="detail-item">
                    <span class="detail-label">Payment Status</span>
                    <span class="detail-value">${order.paymentStatus ? formatStatus(order.paymentStatus) : 'N/A'}</span>
                </div>
                <div class="detail-item">
                    <span class="detail-label">Paid At</span>
                    <span class="detail-value">${order.paidAt ? formatDate(order.paidAt) : 'N/A'}</span>
                </div>
            </div>
        </div>
